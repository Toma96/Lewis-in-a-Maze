            ...
            for artist in self._clickable_artists:
                ...
                if artist.contains(event)[0]:
                    if artist in self._selected_artists:
                        ...
                        # 2) deselect everything else and select only the last selected artist.
                        # Hence we will defer decision until the user releases mouse button.
                        self._clicked_artists.append(artist)
                        self._time_on_key_press = time.time()
...
class PlayableGraph(Graph):

    def __init__(self, *args, **kwargs):
        Graph.__init__(self, *args, **kwargs)

        self.marked_edge = list(self.edge_artists.keys())[0]
        self.edge_artists[self.marked_edge].set_color('red')
        ...
        self.no_vertices = len(self.node_positions)
        self.adjacency_matrix = [[] for i in range(self.no_vertices)]
        ...
        for i in range(self.no_vertices):
            visited = [True if j == i else False for j in range(self.no_vertices)]
            path = [i]
            self._find_hamiltonian_paths(i, visited, path)

        artists = self.node_face_artists.values()
                    ...
                    if artist in self._selected_artists:
                        ...
                        if not self.areConnected(previous_node, next_node) and previous_node is not None:
                            ...
                            if len(self.visited) == self.no_vertices and not self.isSolved():
                                # print("Not a Hamiltonian path! Try again!")
    ...
    def _get_node_by_artist(self, artist):
        try:
            return list(self.node_face_artists.keys())[list(self.node_face_artists.values()).index(artist)]
        except ValueError:
            pass
    ...
    def undo(self):
        if len(self.visited) > 0:
            node = self.visited[-1]
            artist = self.node_face_artists[node]
            artist.set_color('white')
            self._selected_artists.remove(artist)
            if len(self.visited) > 1:
                previous_node = self.visited[-2]
                previous_artist = self.node_face_artists[previous_node]
                previous_artist.set_color('green')
            del self.visited[-1]
            self.fig.canvas.draw_idle()


    def deselect_all_artists(self):
        for artist in self._selected_artists:
            artist.set_color('white')
        self._selected_artists = []
        self.visited = []
    ...
    def _find_hamiltonian_paths(self, start, visited, path):
        if len(path) == self.no_vertices and tuple_in_list(self.marked_edge, path):
            self.hamiltonian_paths.append(path[:])
            return

        for i in self.adjacency_matrix[start]:
            if not visited[i]:
                visited[i] = True
                path.append(i)
    ...
    def isSolved(self):
        return self.visited in self.hamiltonian_paths

    def areConnected(self, previous, next):
        for tup in self.edge_list:
            if previous in tup and next in tup:
                return True
        return False
    ...
    def _delete_nodes(self):
        ...
        nodes = list(self._draggable_artist_to_node.values())
    ...
    def _delete_edges(self):
        ...
        nodes = self._draggable_artist_to_node.values()
...
def tuple_in_list(tup, lista):
    tup_reversed = tup[::-1]
    if tup[0] in lista:
        first_i = lista.index(tup[0])
        try:
            if lista[first_i+1] == tup[1]:
                return True
        except IndexError:
            pass
    if tup_reversed[0] in lista:
        first_i = lista.index(tup_reversed[0])
        try:
            if lista[first_i+1] == tup_reversed[1]:
                return True
        except IndexError:
            return False
    return False
